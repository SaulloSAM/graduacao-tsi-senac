...
namespace WebAppEcommerce.Controllers
{
    ...
    public class ContaController : Controller
    {
        ...
        {
            bool blnValida = Validadores.VerificarCPF(CPFCliente);
            if (blnValida)
        ...
        private Cliente ChecarAcesso(ContaViewModel model)
        {
            try
            {
                Cliente cliente = db.Cliente.Where(m => m.emailCliente == model.emailCliente && m.senhaCliente == model.senhaCliente).FirstOrDefault();
                return cliente;
            }
            catch (Exception ex)
            {
